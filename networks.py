...
class SequenceModel(nnx.Module):
  '''
    Used to produce the next state of the game from the hidden state and the
  '''
  ...
  def __call__(self, hidden_state: chex.Array, encoded_state: chex.Array, action: chex.Array):
    # TODO: Shall we first pass through MLP before the RNN?
    x = jnp.concatenate([encoded_state.reshape(*encoded_state.shape[:-2], -1), action], axis=-1) 
    return self.rnn(hidden_state,x) 
...
def create_optimizers(config: DreamerConfig, game, rngs: nnx.Rngs):
  sequence_optimizer = nnx.Optimizer(
    model= SequenceModel(
