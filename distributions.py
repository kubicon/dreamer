...
def get_normal_log_prob(mean_logits: jax.Array, value: jax.Array, use_symlog=False) ->jax.Array:
   ...
   assert mean_logits.shape == value.shape
...
def get_bin_log_prob(dist_logits: jax.Array, bins: jax.Array,  value: jax.Array, use_symlog = True)->jax.Array:
   ...
   assert dist_logits.shape[-1] == bins.shape[0]
   assert value.shape[:-1] == dist_logits.shape[:-1]
...
def two_hot_encode(bins: jax.Array, value:jax.Array, use_symlog= True) -> jax.Array:
   ...
   #Making use of argmax returning the first occurence
   int_start_idx = jnp.where(jnp.sum(below, axis=-1) == 0, 0, jnp.argmax(below, axis=-1)).astype(jnp.int32)
   int_end_idx = jnp.where(jnp.sum(above, axis=-1) == 0, bins.shape[0] - 1, jnp.argmax(above, axis= -1)).astype(jnp.int32)

   equal = int_start_idx == int_end_idx
   equal_oh = jax.nn.one_hot(int_start_idx, bins.shape[0] - 1, axis=-1)
   
   #Should be shape [Trajectory, Batch, 1]
   start_dist = jnp.abs(value - promoted_bins[int_start_idx][..., None])
   end_dist = jnp.abs(promoted_bins[int_end_idx][..., None] - value) 
   ...
   #[Trajectory, Batch, 2 * bin_range + 1]
   two_hot = jnp.where(equal, equal_oh, start_oh + end_oh)
